...
function hasModeratorRole(token) {
    const claims = token;
...
function splitScenes(storyId, familyId, body, mood, theme) {
...
export const generateStory = onCall(async (request) => {
    ...
    if (!familyId || !childProfileId)
        throw new HttpsError('invalid-argument', 'familyId and childProfileId are required.');
    ...
    for (const scene of scenes)
        await db.collection('storyScenes').doc(scene.id).set(scene);
...
export const createNarrationJob = onCall(async (request) => {
    ...
    if (!familyId || !storyId)
        throw new HttpsError('invalid-argument', 'familyId and storyId are required.');
...
export const reviewModerationEvent = onCall(async (request) => {
    requireAuth(request.auth?.uid);
    if (!hasModeratorRole(request.auth?.token))
        throw new HttpsError('permission-denied', 'Moderator role required.');
    const { eventId, decision, reason } = request.data || {};
    if (!eventId || !['approved', 'rejected', 'escalated', 'resolved'].includes(decision))
        throw new HttpsError('invalid-argument', 'Valid eventId and decision are required.');
