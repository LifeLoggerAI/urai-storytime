...
const UNSAFE_PROMPT_TERMS = ["suicide", "self-harm", "kill", "blood", "weapon", "explicit", "nude", "abuse"];
...
function firstUnsafeTerm(values: string[]) {
  ...
  return UNSAFE_PROMPT_TERMS.find((term) => text.includes(term));
}
  ...
  async function handleCreateStory(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.data.sessionId) {
        throw new Error("Story provider returned no session id.");
      }
    ...
    <main className="storytime-shell">
      <div className="storytime-wrap">
        ...
        <form className="storytime-card storytime-form" onSubmit={handleCreateStory}>
          ...
          {!cloudReady ? (
            <p className="storytime-warning">Cloud generation unavailable. This form opens a labeled deterministic demo until Firebase client env vars, auth, and NEXT_PUBLIC_STORYTIME_CLOUD_MODE=true are configured.</p>
          ) : null}
          <label className="storytime-field">
            Story title
            <input className="storytime-input" value={title} onChange={(event) => setTitle(event.target.value)} maxLength={120} />
          </label>
          <label className="storytime-field">
            Theme
            <input className="storytime-input" value={theme} onChange={(event) => setTheme(event.target.value)} maxLength={80} placeholder="gentle adventure, family memory, quiet signal" />
          </label>
          ...
          <label className="storytime-field">
            ...
            <textarea
              ...
              placeholder="Paste a private moment, weekly reflection, or story seed. Avoid names, addresses, phone numbers, school names, or other sensitive personal details."
            />
          </label>
          ...
          <div className="storytime-actions">
            <button className="storytime-button" type="submit" disabled={Boolean(validationError) || isSubmitting}>
              {cloudReady ? (isSubmitting ? "Creating..." : "Create Story") : "Open Demo Story Session"}
            </button>
          ...
          <p className="storytime-helper">
            Demo mode is local and deterministic. Cloud mode requires Firebase Auth plus a real provider configured in Functions; provider failures are shown without saving partial personal data.
          </p>
        </form>
