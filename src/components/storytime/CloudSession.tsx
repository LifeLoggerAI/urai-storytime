...
import type { EmotionalArcSummary, MemoryScene, NarratorScript, StoryChapter, StoryMoment, StorySession } from "@/lib/storytime/types";
...
async function loadBundle(sessionId: string): Promise<Bundle | null> {
  ...
  const [chapterDocs, momentDocs, sceneDocs, scriptDocs] = await Promise.all([
  ...
  if (state.status === "ready") {
    ...
    return (
      <section className="storytime-stack" aria-label="Cloud Storytime session">
        <StoryPlayer session={bundle.session} chapters={bundle.chapters} moments={bundle.moments} narratorScripts={bundle.scripts} />
        <StoryRevisionEditor
          session={bundle.session}
          chapters={bundle.chapters}
          moments={bundle.moments}
          narratorScripts={bundle.scripts}
        />
