...
export function buildHardOffStorytimeMediaJob(
  ...
  return {
    ...
    providerSpendAuthorized: false,
    publicReleaseAuthorized: false,
    costPolicy: { currency: "USD", maxAuthorizedCost: 0 }
  };
}
...
export function assertStorytimeMediaExecutionHardOff() {
  if (process.env.STORYTIME_MEDIA_EXECUTION === "true") {
    throw new Error(
      "STORYTIME_MEDIA_EXECUTION cannot be enabled until a reviewed worker, provider/cost receipts, cancellation/retry/recovery, private artifact storage, accessibility outputs, deletion handling, and runtime evidence exist."
    );
  }
  return true;
}
