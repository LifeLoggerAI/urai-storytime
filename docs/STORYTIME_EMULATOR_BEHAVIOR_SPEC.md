...
3. Admin can read the dedicated moderation collection; every direct client write, including admin-token writes, is denied.
...
1. Signed-out and cross-user clients cannot read private `storySessions` records.
2. Cross-user clients cannot read owner `storyChapters`, `storyMoments`, `memoryScenes`, `narratorScripts`, or emotional arcs (`emotionalArcSummaries`).
...
5. Non-admin users cannot read moderation records; every client is denied moderation writes. Safety reports are owner/admin-readable and moderation audit logs admin-readable, with all writes server-owned.
