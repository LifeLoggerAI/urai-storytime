...
- **Current evidence:** provider execution is fail-closed behind verified account, explicit consent, request idempotency, quota, audience/locale policy, timeout, output validation and post-generation moderation. Current source additionally requires explicit provider-spend authorization, operator-configured input/output token rates, a bounded output-token ceiling, and a positive per-request dollar ceiling before OpenAI can report ready. A conservative pre-call cost estimate must fit the ceiling; successful responses must include provider request identity and token usage, and the persisted session/request carries a priced provider receipt. Deterministic fallback carries an explicit zero-spend receipt. The provider path remains synchronous and still lacks cancellation/retry/dead-letter orchestration and a cumulative daily dollar budget/reservation ledger.
